...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    // long-running ops (embed, extract, lint, backlinks, reindex, integrity).
    // Pre-fix, each op had its own file-backed checkpoint (or none), which
    // broke on Postgres multi-worker hosts and silently fingerprint-collided
    // across param variations (extract links vs extract timeline shared one
    // file). DB-backed primary; PGLite engine falls back to file-backed at
  ...
  },
  {
    ...
    // v0.36+ autonomous-remediation wave: partial GIN on minion_jobs.data
    // for `data ? 'doctor_run_id'`. Lets `gbrain doctor --remediate` runs
    // be queried by run id for audit trail without sequential-scanning
    // months of cron history. Partial so only doctor-submitted jobs are
    // indexed; ordinary cron submissions don't bloat the index.
    //
    // PGLite has no GIN on JSONB the same way Postgres does, but it ships
    // a compat shim - the CREATE INDEX is accepted and degrades to a noop.
    // Audit-trail queries on PGLite are operator-only and tolerate the
    // sequential scan.
    idempotent: true,
    sql: '',
    sqlFor: {
      ...
      `,
      // PGLite: skip - JSONB GIN partial indexes aren't supported the same
      // way; audit query falls through to sequential scan, which is fine
      // for PGLite's single-host scope.
      pglite: '',
    },
  },
];
